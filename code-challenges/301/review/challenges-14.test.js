...
const toTitleCase = (arr) => {
  return arr.map(item => item.charAt(0).toUpperCase() + item.substr(1));
};
...
let biggerThanLuke = (arr) => {
  let result = "";
  let check = parseInt(arr[0].mass);
  arr.forEach(item=>{
      if(parseInt(item.mass) > check){
...
------------------------------------------------------------------------------------------------ */
const isSecure = (url) => {
  return (url.substring(0,8) === "https://") ? true: false;
}
